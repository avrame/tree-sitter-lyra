...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_lyra_parser", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_lyra_parser": ["*.pyi", "py.typed"],
        "tree_sitter_lyra_parser.queries": ["*.scm"],
    },
    ext_package="tree_sitter_lyra_parser",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_lyra_parser/binding.c",
                "src/parser.c",
            ],
            define_macros=[
